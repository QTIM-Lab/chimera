print("[DEBUG] utils.py loaded from:", __file__)
...
def read_splits(args, fold_idx=None):
    ...
    print(f"?? Looking for splits: {split_names}")
    ...
    for split in split_names:
        ...
        if df.empty:
            raise ValueError(f"? Split file {split_path} is empty!")
...
def get_lr_scheduler(args, optimizer, dataloader):
    ...
    if scheduler_name=='constant':
        ...
        lr_scheduler = get_cosine_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=(len(dataloader) // accum_steps * epochs))
    elif scheduler_name=='linear':
        lr_scheduler = get_linear_schedule_with_warmup(optimizer=optimizer, num_warmup_steps=warmup_steps, num_training_steps=(len(dataloader) // accum_steps) * epochs)
    return lr_scheduler
...
class EarlyStopping:
    def __init__(self, save_dir, patience=20, min_stop_epoch=50, verbose=False, better='min'):
        ...
        self.best_score = np.Inf if better == 'min' else -np.Inf
        self.save_dir = save_dir
        self.early_stop = False
        self.counter = 0

    def is_new_score_better(self, score):
        return score < self.best_score if self.better == 'min' else score > self.best_score
...
def save_checkpoint(config, epoch, model, score, save_dir, fname=None):
    save_state = {'model': model.state_dict(), 'score': score, 'epoch': epoch, 'config': config}
    save_path = j_(save_dir, f'ckpt_epoch_{epoch}.pth') if fname is None else j_(save_dir, fname)
    torch.save(save_state, save_path)

def log_dict_tensorboard(writer, results_dict, tag_prefix, step, verbose=False):
    for key, val in results_dict.items():
        if isinstance(val, (float, int)):
            writer.add_scalar(tag_prefix + key, val, step)
            if verbose:
                print(f"{tag_prefix}{key}: {val:.4f}")
    return writer
